    ...
    class ThreadFolders
    {
        ...
        public void ThreadJob()
        {
            while (Thread.CurrentThread.IsAlive)
            {
                ...
                if (listeFoldersErrors.Count > 0)
                {
                    //SuppressPersoFolderOnError();
                }

                mw.Log("### Opération terminée  ###");
                mw.Log(listeFolders.Count + " dossiers créés, " + listeFoldersErrors.Count + " erreurs durant la création. Les dossiers en erreur ont été supprimés.");
        ...
        private void CreatePublic(string publicFolder, Folder folder)
        {
            ...
            //Create the new rules for the directory
            dirSecurity.AddAccessRule(new FileSystemAccessRule(@"ad\fbm-" + departement + "-g", FileSystemRights.FullControl & ~FileSystemRights.DeleteSubdirectoriesAndFiles & ~FileSystemRights.TakeOwnership & ~FileSystemRights.ChangePermissions, InheritanceFlags.ContainerInherit | InheritanceFlags.ObjectInherit, PropagationFlags.NoPropagateInherit, AccessControlType.Allow));
            dirSecurity.AddAccessRule(new FileSystemAccessRule(@"ad\fbm-" + departement + "-admin-acl-g", FileSystemRights.FullControl, InheritanceFlags.ContainerInherit | InheritanceFlags.ObjectInherit, PropagationFlags.NoPropagateInherit, AccessControlType.Allow));
            Directory.SetAccessControl(publicFolder, dirSecurity);
        }

        private void SuppressPersoFolderOnError()
        {
